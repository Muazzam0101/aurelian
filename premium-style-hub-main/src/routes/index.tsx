    ...
    <Layout>
      ...
      <Container className="py-16">
        ...
        <div className="grid lg:grid-cols-3 gap-5">
          <a href={`/product/${topPicksMain.id}`} className="lg:row-span-2 group relative rounded-3xl overflow-hidden bg-ink min-h-[480px]">
          ...
          {topPicks.map(p => <ProductCard key={p.id} p={p} compact />)}
        </div>
      </Container>
      ...
      <Container className="py-12">
        <div className="grid lg:grid-cols-4 gap-5">
          <div className="lg:col-span-1 rounded-3xl bg-ember text-white p-8 flex flex-col justify-between min-h-[300px]">
            <div>
            ...
            <a href="/shop" className="mt-6 h-11 self-start px-6 rounded-full bg-ink text-background text-xs uppercase tracking-widest">Shop Sale</a>
          </div>
          {promoGrid.map(p => <ProductCard key={p.id} p={p} compact />)}
        </div>
      </Container>
      ...
      <section className="bg-ink text-background py-16">
        <Container>
          <div className="flex items-end justify-between mb-6">
            ...
            <a href="/shop" className="text-sm border-b border-background pb-0.5">View all</a>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {hot.map(p => (
              <div key={p.id} className="rounded-2xl overflow-hidden bg-background text-foreground">
                <ProductCard p={p} compact />
              </div>
            ))}
          </div>
      ...
      <Container className="py-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {watches.map(p => <ProductCard key={p.id} p={p} compact />)}
        </div>
      </Container>
      ...
      <section className="bg-beige py-16">
        <Container>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {shoes.map(p => <ProductCard key={p.id} p={p} compact />)}
          </div>
        </Container>
      </section>
      ...
      <Container className="py-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {newArrGrid.map(p => <ProductCard key={p.id} p={p} compact />)}
        </div>
      ...
      <Container className="py-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {finalGrid.map(p => <ProductCard key={p.id} p={p} compact />)}
        </div>
      </Container>
    </Layout>
  );
