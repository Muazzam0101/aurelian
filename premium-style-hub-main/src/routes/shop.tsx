      ...
      <Container className="py-10">
        <div className="flex items-end justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-2">{filteredItems.length} pieces · curated weekly</p>
          </div>
          <div className="hidden md:flex items-center gap-2 text-sm">
            <span className="text-muted-foreground">Sort by</span>
            <select className="h-10 px-3 rounded-full bg-beige border border-border text-sm">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {items.map((p, i) => (
                  <React.Fragment key={p.id}>
                    ...
                    {i === 7 && (
                      <div key="promo1" className="rounded-2xl bg-ink text-background p-6 flex flex-col justify-between">
                        <p className="text-[11px] uppercase tracking-[0.22em] text-gold">Atelier Pass</p>
                        <h3 className="font-display text-2xl mt-3">Members save 15% on every order</h3>
                        <a href="#" className="mt-4 self-start text-xs uppercase tracking-widest border-b border-background">Join free</a>
                      </div>
                    )}
                    {i === 17 && (
                      <div key="promo2" className="rounded-2xl bg-beige p-6 flex flex-col justify-between">
                        <p className="text-[11px] uppercase tracking-[0.22em] text-ember">Free engraving</p>
                        <h3 className="font-display text-2xl mt-3 text-ink">Personalise any timepiece, complimentary.</h3>
